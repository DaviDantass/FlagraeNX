...
void hex_dump(const uint8_t *data, size_t size) {
    size_t limit = size > HEXDUMP_SIZE ? HEXDUMP_SIZE : size;
...
int main(void) {
    ...
    if (pcap_findalldevs(&interfaces, error_buffer) == -1) {
        fprintf(
            stderr,
            "Erro ao encontrar interfaces: %s\n",
            error_buffer
        );
    ...
    if (datalink != DLT_EN10MB) {
        printf(
            "Aviso: link-layer nao e Ethernet (DLT=%d).\n",
            datalink
        );
    }
    ...
    while (1) {
        ...
        if (
            packet_header->caplen <
            ETHERNET_HEADER_SIZE + ip_header_length
        ) {
            continue;
        }
        ...
        if (ip->protocol == 6) {
            ...
            if (payload_offset < packet_header->caplen) {
                size_t payload_size =
                    packet_header->caplen - payload_offset;
